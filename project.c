...
void viewNotifications(const char* username) {
    ...
    printf("\nDo you want to clear your notifications? (y/n): ");
    char ch;
    scanf(" %c", &ch);
    if (tolower(ch) == 'y') {
        FILE *notifFile = fopen("notifications.txt", "r");
        FILE *tempFile = fopen("temp_notifications.txt", "w");
        char line[256];

        if (notifFile && tempFile) {
            ...
            rename("temp_notifications.txt", "notifications.txt");
            printf("Notifications cleared.\n");
        } else {
            if (notifFile) fclose(notifFile);
            if (tempFile) fclose(tempFile);
            printf("Failed to clear notifications.\n");
        }
    }
}
